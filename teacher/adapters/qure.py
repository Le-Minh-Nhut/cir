...
def run_batch(model, txt_processor, preprocess, cases: list[dict], image_root: Path, device: torch.device) -> dict[str, object]:
    ...
    reference_states = encode_reference(model=model, images=reference_images,)
...
def parse_args():
    ...
    parser.add_argument("--config", type=Path, default=Path("teacher/repos/QuRe/configs/fashionIQ/eval.json"),)

...
def resolve_image_path(
    ...
    for path in candidates:
        if path.exists():
            return path
    raise FileNotFoundError(f"Could not find image ID={image_id} category={category} under {image_root}")
...
def build_hint(hint_root: Path, checkpoint_path: Path, device: torch.device):
    ...
    if isinstance(loaded, torch.nn.Module):
        ...
        print(f"HINT state-dict fallback: missing={len(load_result.missing_keys)} unexpected={len(load_result.unexpected_keys)}")
...
def load_correction_dicts(correction_root: Path) -> dict[str, dict[str, str]]:
    ...
    for category in ("dress", "shirt", "toptee"):
        ...
        if not path.exists():
            raise FileNotFoundError(f"Missing HINT FashionIQ correction dictionary: {path}")
...
def encode_reference(model, reference_images: torch.Tensor) -> torch.Tensor:
    with model.maybe_autocast():
        reference_embeds = model.ln_vision(model.visual_encoder(reference_images))
    return reference_embeds.float()

@torch.no_grad()
def compose_query_from_reference(model, reference_embeds: torch.Tensor, texts: list[str], txt_processor) -> tuple[torch.Tensor, torch.Tensor]:
    device = reference_embeds.device
    texts = [txt_processor(text) for text in texts]
    image_atts = torch.ones(reference_embeds.size()[:-1], dtype=torch.long, device=device)
    ...
    if query_tokens.size(1) != 32:
        raise ValueError(f"HINT native retrieval expects 32 query tokens, got {query_tokens.size(1)}")
    ...
    query_pre_norm = model.text_proj(fusion_output.last_hidden_state[:, query_tokens.size(1), :])
    query_normalized = F.normalize(query_pre_norm, dim=-1)
    return query_pre_norm, query_normalized

@torch.no_grad()
def compose_query(model, reference_images: torch.Tensor, texts: list[str], txt_processor) -> tuple[torch.Tensor, torch.Tensor]:
    reference_embeds = encode_reference(model=model, reference_images=reference_images)
    return compose_query_from_reference(
        model=model,
...
def run_batch(model, txt_processor, preprocess, correction_dicts, cases, image_root, device):
    ...
    reference_embeds = encode_reference(
        model=model,
        reference_images=reference_images,
    )
    q_full_pre, q_full = compose_query_from_reference(
        model=model,
        reference_embeds=reference_embeds,
        texts=full_texts,
        txt_processor=txt_processor,
    )
    q_minus_1_pre, q_minus_1 = compose_query_from_reference(
        model=model,
        reference_embeds=reference_embeds,
        texts=minus_1_texts,
        txt_processor=txt_processor,
    )
    q_minus_2_pre, q_minus_2 = compose_query_from_reference(
        model=model,
        reference_embeds=reference_embeds,
        texts=minus_2_texts,
        txt_processor=txt_processor,
...
def validate_outputs(outputs):
    ...
    if outputs["q_full"].shape[-1] != 256:
        raise ValueError(f"Expected HINT query dim 256, got {outputs['q_full'].shape[-1]}")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help=("FashionIQ image root containing dress/, shirt/, toptee/ subdirectories (or the upstream resized_image root)."),
    )
    parser.add_argument(
        "--correction-root",
        type=Path,
        required=True,
        help=("Directory containing correction_dict_dress.json, correction_dict_shirt.json, correction_dict_toptee.json"),
    )
    parser.add_argument(
        "--output",
        type=Path,

...
class ActionClaimBindingLoss(nn.Module):
    ...
    def forward(self, intents: Tensor, claimed_semantics: Tensor) -> Tensor:
        ...
        actions = normalize_fp32(self.action_projection(intents), dim=-1)
        claims = normalize_fp32(self.claim_projection(claimed_semantics), dim=-1)
        # Similarity logits and cross-entropy stay FP32 under AMP.
        with torch.autocast(device_type=intents.device.type, enabled=False):
            similarities = (
                torch.einsum("bkd,bjd->bkj", actions.float(), claims.float())
                / self.temperature
            )
            labels = torch.arange(intents.shape[1], device=intents.device).expand(
                intents.shape[0], -1
            )

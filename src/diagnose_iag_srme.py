...
def parameter_movement_from_fresh(model: nn.Module, checkpoint_state: Mapping[str, Tensor]):
    ...
    for group, slot in acc.items():
        result[group] = {
            "relative_l2_change": math.sqrt(slot["diff_sq"]) / max(math.sqrt(slot["base_sq"]), 1e-12),
...
def gradient_group_stats(model: nn.Module, objective: nn.Module):
    acc = defaultdict(lambda: dict(grad_sq=0.0, grad_max=0.0, numel=0.0, grad_numel=0.0,
                                   nonzero_numel=0.0, trainable_tensors=0.0, no_grad_tensors=0.0))
    ...
    for group, slot in acc.items():
        result[group] = {
            ...
            "no_grad_tensor_fraction": slot["no_grad_tensors"] / max(slot["trainable_tensors"], 1.0),
    ...
    if backbone is not None:
        ...
        if inner is not None:
            candidates += [
                ...
                ("backbone_visual_projection", first_trainable(getattr(inner, "visual_projection", None))),
            ]
        candidates.append(("backbone_text_adapter", first_trainable(getattr(backbone, "text_adapter", None))))
    seen = set()
...
def checkpoint_summary(checkpoint: Mapping[str, Any]):
    ...
    if isinstance(optimizer, dict):
        ...
        optimizer_summary = {
            ...
            "weight_decays": [float(g["weight_decay"]) for g in groups if isinstance(g, dict) and "weight_decay" in g],
        }
    return {
        "epoch": checkpoint.get("epoch"), "metric": checkpoint.get("metric"),
        "architecture": metadata.get("architecture"), "experiment_identity": metadata.get("experiment_identity"),
        "global_readout_mode": metadata.get("global_readout_mode"), "finetune_policy": metadata.get("finetune_policy"),
        "train_vision": metadata.get("train_vision"), "train_text": metadata.get("train_text"),
        "train_text_projection": metadata.get("train_text_projection"), "precision": metadata.get("precision"),
        "objective_config": metadata.get("objective_config"), "model_config": metadata.get("model_config"),
...
def build_objective(config: ObjectiveConfig, model: nn.Module, tokenizer: object,
                    train_dataset: FashionIQDataset, max_text_length: int):
    vocabulary = None
    prototypes = None
    ...
    return IAGSRMEObjective(config, state_dim=model.backbone.state_dim,
                           concept_vocabulary=vocabulary, concept_prototypes=prototypes)


def analyze_batch(model: nn.Module, objective: IAGSRMEObjective, output: Mapping[str, Any],
                  losses: Mapping[str, Tensor], targets: Tensor,
    ...
    raw = {
        "terminal": scalar(losses["terminal"]), "pair": scalar(losses["pair"]),
    ...
    for name in [
        "concept_positive_recall", "concept_negative_false_positive_rate", "instruction_concept_coverage",
        "prototype_occupancy", "prototype_entropy", "prototype_pairwise_cosine",
        "functional_pairwise_cosine", "functional_rank", "mean_delta_q_norm", "stop_rate",
        "mean_rollout_length", "useful_candidate_count", "dpp_valid_timestep_count", "dpp_valid_rate",
    ...
    record["retrieval/terminal_loss_improvement"] = scalar(initial_terminal) - scalar(losses["terminal"])
    ...
    record["rollout/final_state_relative_change"] = float((state_diff.norm(dim=-1) / state_base.norm(dim=-1).clamp_min(1e-8)).mean().cpu())
    record["rollout/final_initial_query_cosine"] = float(F.cosine_similarity(query.detach().float(), initial_query.detach().float(), dim=-1).mean().cpu())

    buckets = defaultdict(list)
    teacher_all, predicted_all = [], []
    for step in output["steps"]:
        tensors = {k: step[k] for k in ["proposals", "entities", "actions", "grounding", "alpha_read", "exec_mask",
                                                "delta", "candidate_states", "candidate_queries", "delta_q", "scores",
                                                "fuse_gamma", "fuse_beta"]}
        for name, tensor in tensors.items():
            buckets[f"finite/{name}"].append(float(finite_tensor(tensor)))

        for prefix, tensor in [("proposal", step["proposals"]), ("entity", step["entities"]),
                               ("action", step["actions"]), ("delta_q", step["delta_q"]),
        ...
        for prefix, tensor in [("proposal", step["proposals"]), ("entity", step["entities"]), ("action", step["actions"])]:
            buckets[f"{prefix}/norm"].append(float(tensor.detach().float().norm(dim=-1).mean().cpu()))
        ...
        buckets["grounding/exec_mask_fraction_gt_0_5"].append(float((mask > 0.5).float().mean().cpu()))
        buckets["grounding/exec_mask_fraction_lt_0_05"].append(float((mask < 0.05).float().mean().cpu()))
        buckets["grounding/exec_mask_fraction_gt_0_95"].append(float((mask > 0.95).float().mean().cpu()))
        ...
        saturation = (((gamma < 0.05) | (gamma > 0.95)).float().mean() + ((beta < 0.05) | (beta > 0.95)).float().mean()) / 2
        ...
        buckets["executor/delta_relative_parent"].append(float((delta_norm / parent.norm(dim=-1)[:, None].clamp_min(1e-8)).mean().cpu()))
        buckets["executor/candidate_state_pairwise_l2"].append(pairwise_l2_mean(step["candidate_states"]))
        ...
        buckets["effect/delta_q_near_zero_fraction"].append(float((dq_norm < 1e-5).float().mean().cpu()))
        ...
        buckets["score/range"].append(float((scores.max(dim=-1).values - scores.min(dim=-1).values).mean().cpu()))
        ...
        teacher, valid = marginal_teacher_utilities(step["current_query"], step["candidate_queries"], targets.detach(),
                                                    pos_live, neg_live, cfg.retrieval_temperature)
        if valid.any():
            ...
            buckets["teacher/positive_candidate_fraction"].append(float((teacher > 0).float().mean().cpu()))
            quality = torch.sigmoid(teacher / cfg.tau_dpp)
            buckets["teacher/dpp_useful_candidate_count"].append(float((quality > cfg.useful_threshold).sum(dim=-1).float().mean().cpu()))

            left, right = torch.triu_indices(teacher.shape[1], teacher.shape[1], offset=1, device=teacher.device)
            ...
            if confident.any():
                ...
                buckets["score/pairwise_teacher_accuracy"].append(float((correct.float() + 0.5 * ties.float()).mean().cpu()))
            ...
            buckets["decision/harmful_execution_rate"].append(float(((~stop) & (selected_u < 0)).float().mean().cpu()))
            buckets["decision/missed_opportunity_stop_rate"].append(float((stop & (oracle_u > 0)).float().mean().cpu()))
            should_stop = oracle_u <= 0
            buckets["decision/stop_execute_accuracy"].append(float((stop == should_stop).float().mean().cpu()))
            exact = torch.where(should_stop, stop, (~stop) & (gather_idx == oracle_idx))
            buckets["decision/exact_oracle_action_accuracy"].append(float(exact.float().mean().cpu()))
    ...
    if teacher_all:
        record["score/global_teacher_pearson"] = pearson(torch.cat(predicted_all), torch.cat(teacher_all))
    return record, selection


def component_gradient_routing(model: nn.Module, objective: IAGSRMEObjective, losses: Mapping[str, Tensor]):
...
def build_health_flags(agg: Mapping[str, float], gradient: Mapping[str, Any], config_diff: Mapping[str, Any]):
    flags = []
    diffs = config_diff.get("differences", {})
    if diffs:
        flag(flags, "WARN", "OBJECTIVE_CONFIG_MISMATCH", f"Current config differs from checkpoint: {', '.join(sorted(diffs))}.")
    ...
    if dq is not None and dq < 1e-5:
        flag(flags, "FAIL", "RETRIEVAL_EFFECT_NEAR_ZERO", f"mean delta_q norm={dq:.3e}; Executor is nearly invisible to retrieval.")
    ...
    if useful is not None and useful < 2:
        flag(flags, "WARN", "DPP_MOSTLY_GATED_OFF", f"useful candidates={useful:.2f}; DPP needs >=2, so lambda_dpp may not matter.")
    ...
    if acc is not None and acc < 0.55:
        flag(flags, "WARN", "SCORENET_WEAK_RANKING", f"ScoreNet pairwise teacher accuracy={acc:.3f}, near chance.")
    ...
    if harmful is not None and harmful > 0.20:
        flag(flags, "WARN", "HARMFUL_EXECUTIONS", f"{harmful:.1%} selected executions have negative teacher utility.")

    retrieval_gain = agg.get("retrieval/terminal_loss_improvement")
    if retrieval_gain is not None and retrieval_gain <= 0:
        flag(flags, "FAIL", "ROLLOUT_DOES_NOT_IMPROVE_RETRIEVAL", f"Initial->final terminal improvement={retrieval_gain:.5f}.")
    ...
    if concept_fpr is not None and concept_fpr > 0.50:
        flag(flags, "WARN", "HIGH_CONCEPT_FALSE_POSITIVE_RATE", f"Concept negative FPR={concept_fpr:.3f}.")
    ...
    if gradient:
        ...
        for name in ("proposal", "grounder", "action_fusion", "executor", "score_net"):
            ...
            if isinstance(info, Mapping) and info.get("trainable_numel", 0) > 0 and info.get("grad_l2", 0) == 0:
                flag(flags, "FAIL", f"ZERO_GRAD_{name.upper()}", f"{name} is trainable but gets zero total gradient.")
        ...
        if isinstance(dpp_route, Mapping) and dpp_route.get("executor", 0.0) == 0.0:
            flag(flags, "WARN", "DPP_NOT_REACHING_EXECUTOR_PROBE", "DPP gradient to representative Executor parameter is zero on this batch; usually the DPP guard is inactive.")

    if not flags:
        flag(flags, "OK", "NO_MAJOR_AUTOMATIC_FLAGS", "No major issue crossed conservative thresholds; inspect full report anyway.")
    return flags
...
def render_markdown(report: Mapping[str, Any]):
    ...
    lines = ["# IAG-SRME V2 R0 Diagnostic Report", "",
             "## Checkpoint", "",
    ...
    sections = {
        "Retrieval / rollout": ["retrieval/initial_terminal_loss", "retrieval/final_terminal_loss", "retrieval/terminal_loss_improvement",
                                "retrieval/positive_similarity_gain", "retrieval/inbatch_rank_improvement", "rollout/final_state_relative_change",
                                "rollout/final_initial_query_cosine", "objective/mean_rollout_length", "objective/stop_rate"],
        "Collapse / diversity": ["step/proposal/pairwise_cosine_mean", "step/entity/pairwise_cosine_mean", "step/action/pairwise_cosine_mean",
                                 "objective/functional_pairwise_cosine", "objective/functional_rank", "objective/mean_delta_q_norm",
                                 "step/effect/delta_q_effective_rank", "step/effect/delta_q_near_zero_fraction", "step/executor/candidate_state_pairwise_l2"],
        "Grounding / write masks": ["step/grounding/alpha_entropy", "step/grounding/alpha_peak", "step/grounding/exec_mask_mean",
                                    "step/grounding/exec_mask_fraction_gt_0_5", "step/grounding/exec_mask_fraction_lt_0_05",
                                    "step/grounding/exec_mask_fraction_gt_0_95", "step/grounding/exec_mask_jaccard",
                                    "step/fusion/gamma_mean", "step/fusion/beta_mean", "step/fusion/gate_saturation"],
        "ScoreNet / teacher": ["score/global_teacher_pearson", "step/score/pairwise_teacher_accuracy", "step/decision/selected_teacher_utility",
                               "step/decision/oracle_teacher_utility", "step/decision/oracle_regret", "step/decision/harmful_execution_rate",
                               "step/decision/missed_opportunity_stop_rate", "step/decision/stop_execute_accuracy", "step/decision/exact_oracle_action_accuracy"],
    ...
    if gradient:
        ...
        rows = [[g, v.get("grad_l2", float("nan")), v.get("grad_max_abs", float("nan")),
                 v.get("nonzero_grad_element_fraction", float("nan")), v.get("no_grad_tensor_fraction", float("nan"))]
                for g, v in sorted(gradient.get("total_gradient_groups", {}).items())]
        lines += [table(rows, ["Module", "grad L2", "max |grad|", "nonzero elem frac", "no-grad tensor frac"]), ""]
    ...
    if movement:
        rows = [[g, v["relative_l2_change"], v["mean_abs_change"], v["max_abs_change"], int(v["numel"])] for g, v in sorted(movement.items())]
        lines += ["## Approximate checkpoint movement from fresh initialization", "",
                  table(rows, ["Group", "relative L2 change", "mean abs change", "max abs change", "numel"]), "",
    ...
    if diffs:
        lines += ["## Current vs checkpoint objective config", "",
                  table([[k, v.get("current"), v.get("checkpoint")] for k, v in diffs.items()], ["Field", "Current", "Checkpoint"]), ""]
    lines += ["## Selection histogram", "", f"`{json.dumps(report['selection_histogram'], sort_keys=True)}`", "",
              "## All aggregate metrics", "", table([[k, v] for k, v in sorted(agg.items())], ["Metric", "Mean"]), "",
...
def main(cfg: DictConfig):
    ...
    train_dataset = FashionIQDataset(annotation_root, "train", CATEGORIES,
                                     caption_policy=str(cfg.experiment.train_caption_policy), seed=int(cfg.seed))
    ...
    objective = build_objective(active_obj, model, tokenizer, train_dataset, int(cfg.backbone.max_text_length)).to(device)
    if "objective" in checkpoint:
        missing, unexpected = objective.load_state_dict(checkpoint["objective"], strict=False)
        if missing or unexpected:
            print("[diagnostic] objective state mismatch", {"missing": missing, "unexpected": unexpected})

    collator = FashionIQImageCollator(image_store, tokenizer, processor, int(cfg.backbone.max_text_length), include_targets=True)
    loader = DataLoader(train_dataset, batch_size=diagnostic_batch_size, shuffle=True,
                        num_workers=int(cfg.experiment.num_workers), pin_memory=True,
                        collate_fn=collator, drop_last=True,
                        generator=torch.Generator().manual_seed(int(cfg.seed) + 913))
    ...
    report = {
        "checkpoint_path": str(checkpoint_path), "checkpoint": checkpoint_summary(checkpoint),
        "active_objective_config": asdict(active_obj), "current_objective_config": asdict(current_obj),
        "objective_config_comparison": comparison,
        "diagnostic": {"batches": len(records), "batch_size": diagnostic_batch_size, "precision": str(cfg.runtime.precision),
                       "device": str(device), "deep_grad": deep_grad, "use_checkpoint_objective": use_checkpoint_objective},
        "aggregate": aggregate, "per_batch": records,
        "selection_histogram": {str(k): int(v) for k, v in sorted(selections.items())},
        "gradient": gradient, "parameter_movement_from_fresh": movement,
        "peak_cuda_memory": memory, "health_flags": health,
    }

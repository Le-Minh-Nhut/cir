...
def load_encoder_correction_dicts(
...
def build_fashioniq_encoder_model(cfg: DictConfig, device: torch.device) -> TAPER:
...
def main(cfg: DictConfig) -> None:
    if str(cfg.dataset.get("name", "")) != "fashioniq":
        raise ValueError("src/train_fashioniq_encoder.py requires dataset=fashioniq")
    if str(cfg.protocol.get("name", "")) != "fashioniq_encoder":
        raise ValueError(
            "src/train_fashioniq_encoder.py requires protocol=fashioniq_encoder"
        )
    if str(cfg.experiment.get("name", "")) != "taper_e2e":
        raise ValueError("src/train_fashioniq_encoder.py requires experiment=taper_e2e")
    ...
    cache_root = Path(cfg.paths.cache_root) / "fashioniq" / "csmcir"
    ...
    train_dataset = FashionIQDataset(
        ...
        split="train",
        categories=FASHIONIQ_ENCODER_CATEGORIES,
    ...
    output_dir = Path(cfg.paths.output_root) / "fashioniq" / "encoder"

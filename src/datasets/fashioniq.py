...
def normalize_fashioniq_caption(caption: str, correction_dict: Mapping[str, str] | None = None) -> str:
    ...
    punctuation_to_space = str.maketrans(punctuation_map)

    tokens = str(caption).lower().translate(punctuation_to_space).strip().split()
    corrected_tokens: list[str] = []
    for token in tokens:
        if correction_dict is not None and token in correction_dict:
            corrected_token = correction_dict[token]
        else:
